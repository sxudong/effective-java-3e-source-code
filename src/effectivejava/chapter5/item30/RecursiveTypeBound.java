...
public class RecursiveTypeBound { // 普通类（非泛型方法）
    ...
    // 返回集合中的最大值-使用递归类型限定
    public static <E extends Comparable<E>> E max(Collection<E> c) { // 类型限制 <E extends Comparable<E>> 可以读作“任何可以与自已比较的类型E” P108
        if (c.isEmpty())
        ...
        for (E e : c)
            ...
            if (result == null || e.compareTo(result) > 0) { // 1>0 2>1 3>2 递归与result比较所有数字
